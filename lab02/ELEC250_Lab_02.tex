...
\begin{equation}
	\tan { \theta  } =\left( \frac { 1 }{ \omega RC }  \right) =\left( \frac { 1 }{ 2\pi fRC }  \right) 
	\label{eqn:rc_theta}
\end{equation}

\begin{equation}
		I_{max}(t) = V_{max}(t)\frac{C\omega}{\sqrt{(RC\omega)^2 + 1} }
		\label{eqn:rc_current}
\end{equation}

Hey, look at equation~\eqref{eqn:rc_theta}.
...
\begin{table}[h]
	...
	\begin{tabular}{c c c c c}
		...
		\cline{2-5}
		$R$ (k$\Omega$)	& $I$ (A) 	& $\theta$ ($^\circ$) 	& $I$ (A) 	& $\theta$ ($^\circ$) \\
		\hline
		1				& -			& 72.56					& -			&	-	\\
		5				& -			& 32.48					& -			&	-	\\
		10				& -			& 17.67					& -			&	-	\\
		\toprule
	\end{tabular}
	\caption{Calculated and measured values in the RC circuit ($f$ = 10 kHz)}
	\label{table:rc}
\end{table}
...
\begin{equation}
	\tan { \theta  } =\left( \frac { -\omega L }{ R }  \right) =\left( \frac { -2\pi fL }{ R }  \right) 
	\label{eqn:rl_theta}
\end{equation}
...
\begin{table}[h]
	...
	\begin{tabular}{c c c c c c c c c}
		...
		\cline{2-9}
		$R$ (k$\Omega$)	& $I$ (mA) 	& $\theta$ ($^\circ$) 	& $I$ (mA) 	& $\theta$ ($^\circ$)	& $I$ (mA) 	& $\theta$ ($^\circ$)	& $I$ (mA) 	& $\theta$ ($^\circ$) \\
		\hline
		1		& 10.0	& -0.18	& 8.23	& -34.65& 5.61	& -55.89& 3.03	& -72.34\\
		5		& 2.00	& -0.04	& 1.98	& -7.87	& 1.92	& -16.45& 1.69	& -32.14\\
		10		& 1.00	& -0.02	& 0.998	& -3.95	& 0.989	& -8.40	& 0.954	& -17.44\\
		\toprule
	\end{tabular}
	\caption{Calculated values in the RL circuit ($f$ = 500 kHz)}
	\label{table:rl_calc}
\end{table}
...
\begin{table}[h]
	...
	\begin{tabular}{c c c c c c c c c}
		...
		\toprule
	\end{tabular}
	\caption{Measured values in the RL circuit ($f$ = 500 kHz)}
	\label{table:rl_meas}
\end{table}
